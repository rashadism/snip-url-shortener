...
Schema is in `api-service/schema.sql`, embedded into the api-service binary via `//go:embed` and applied on startup by `Store.Migrate` (api-service/migrate.go). All `CREATE TABLE`/`CREATE INDEX` statements are `IF NOT EXISTS`, so the migration is idempotent. Two tables: `urls` (with click_count denormalized) and `clicks` (event log).
